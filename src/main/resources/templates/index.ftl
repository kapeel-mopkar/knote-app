...
<form action="/note" method="POST" enctype="multipart/form-data">
    <ol class="list pl0">
        ...
        <li class="mv3"><label class="f6 b db mb2" for="description">Write your content here</label>
            ...
            <input name="fileName" type="hidden" value="<#if uploadedfilename??>${uploadedfilename}</#if>" />
    ...
    <#if notes??>
        <#list notes as note>
            <li class="mv3 bb bw2 b--light-yellow bg-washed-yellow ph4 pv2"><p class="measure"></p>
                <div>
                	<span><#if (note.getFileName())?has_content><img src="/downloadFile?fileName=${note.getFileName()}" alt="${note.getDescription()}" width="80" height="80"></#if></span>
                	<span>${note.getDescription()}</span>
                </div>
            </li>
        <#else>
